...
module ExtremeStartup
  describe SquareCubeQuestion do
    ...
    it "converts to a string" do
      question.as_text.should =~ /hvilke av disse tallene har heltalls kvadratrot og kubikkrot: (\d+, )+(\d+)/i
    end
    ...
    context "when the numbers are known" do
      ...
      it "converts to the right string" do
        question.as_text.should =~ /hvilke av disse tallene har heltalls kvadratrot og kubikkrot: 62, 63, 64/i
      end
